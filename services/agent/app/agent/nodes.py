...
async def agent_node(state: AgentState, llm_with_tools: BaseChatModel, config=None):
    ...
    # 如果检测到图片，使用 VLM 生成
    if has_images and config and config.get("enable_multimodal"):
        try:
            # 创建 VLM Service
            vlm_service = VLMService(
                api_key=config.get("dashscope_api_key", ""),
                model_name=config.get("multimodal_llm_model", "qwen-vl-max")
            )
...
请仔细查看图片中的内容，结合文本信息，给出准确、详细的回答。"""

                answer = vlm_service.analyze_image(
